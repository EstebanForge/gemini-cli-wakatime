...
class Dependencies {
    ...
    getCliLocation() {
        ...
        const ext = (0, utils_1.isWindows)() ? '.exe' : '';
    ...
    getCliLocationGlobal() {
        ...
        const binaryName = `wakatime-cli${(0, utils_1.isWindows)() ? '.exe' : ''}`;
    ...
    isCliLatest(callback) {
        ...
        let args = ['--version'];
        const options = (0, utils_1.buildOptions)();
        try {
    ...
    getLatestCliVersion(callback) {
        ...
        let options = {
            ...
            json: true,
            headers: {
                'User-Agent': 'github.com/EstebanForge/gemini-cli-wakatime',
            },
        };
    ...
    reportMissingPlatformSupport(osname, architecture) {
        const url = `https://api.wakatime.com/api/v1/cli-missing?osname=${osname}&architecture=${architecture}&plugin=gemini-cli`;
        const proxy = this.options.getSetting('settings', 'proxy');
        const noSSLVerify = this.options.getSetting('settings', 'no_ssl_verify');
        let options = { url: url };
        if (proxy)
